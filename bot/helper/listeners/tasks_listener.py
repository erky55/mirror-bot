...
class MirrorLeechListener:
    ...
    async def onDownloadComplete(self):
        ...
        if self.isLeech:
            size = await get_path_size(up_dir)
            for s in list(excluded_files.values()):
                size = size - s
    ...
    async def onUploadComplete(self, link, size, files, folders, mime_type, name, rclonePath='', dir_id='', private=False):
        msg = f"<b>Name:</b> <copy>{escape(name)}</copy>\n\n<b>Size:</b> {get_readable_file_size(size)}"
...
#               msg += f'\n<b>Corrupted Files:</b> {files}'
            ...
            if link or rclonePath and config_dict['RCLONE_SERVE_URL']:
                ...
                if not rclonePath and dir_id:
                    INDEX_URL = ''
                    if private:
                        INDEX_URL = self.user_dict['index_url'] if self.user_dict.get(
                            'index_url') else ''
